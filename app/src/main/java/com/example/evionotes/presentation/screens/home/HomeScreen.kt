        ...
        when (authState) {
            ...
            is AuthState.LoggedIn -> {
                ...
                Column(
                    ...
                    when {
                        displayNotes?.isEmpty() == true && searchQuery.isNotBlank() -> {
                            // No search results
                            Box(
                                modifier = Modifier.fillMaxSize(),
                                contentAlignment = Alignment.Center
                        ...
                        notes.itemCount == 0 && searchQuery.isBlank() -> {
                            // No notes yet
                            Box(
                                modifier = Modifier.fillMaxSize(),
                                contentAlignment = Alignment.Center
                                ...
                                if (searchQuery.isNotBlank() && displayNotes != null) {
                                    items(displayNotes.size) { index ->
                                        NoteCard(
                                            ...
                                            onClicks = { onNavigateToNoteDetail(displayNotes[index].id) },
                                            onPin = { viewModel.togglePinNote(displayNotes[index].id) },
                                            onArchive = { viewModel.toggleArchiveNote(displayNotes[index].id) },
                                            onDelete = { viewModel.deleteNote(displayNotes[index].id) }
                                        )
                                    }
                                } else {
                                    items(
                                        ...
                                        notes[index]?.let { note ->
                                            NoteCard(
                                                note = note,
                                                onClicks = { onNavigateToNoteDetail(note.id) },
                                                onPin = { viewModel.togglePinNote(note.id) },
                                                onArchive = { viewModel.toggleArchiveNote(note.id) },
            ...
            AuthState.LoggedOut -> {} // already handled by LaunchedEffect
        }
    }
}